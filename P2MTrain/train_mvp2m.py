...
def main(cfg):
    ...
    data = DataFetcher(file_list=cfg.train_file_path, data_root=cfg.train_data_path, image_root=cfg.train_image_path,
                       is_val=False)
    data.setDaemon(True)
    data.start()
    # ---------------------------------------------------------------   
    # Load init ellipsoid and info about vertices and edges
    print("=================================================> ellipsoid loading:加载椭圆体")
    ...
    if pre_train:
        ...
        model = GCN.GCN_model(placeholders_features=features, num_features_nonzero=num_features_nonzero,
                              placeholder_dropout=dropout,
                              pool_idx=pool_idx, args=cfg, support1=support1, support2=support2, support3=support3,
                              lape_idx=lape_idx, edges=edges, faces_triangle=faces_triangle)
    print('模型加载完成')
    # -------------------------------
    ...
    pre_weight = True
    if pre_weight == True:
        model.load_weights('/home/giga/Desktop/davistao/p2m_tf2/Pixel2Mesh-Tensorflow2/results/coarse_mvp2m/models/20200223model_weights/epoch1')
    # 权重保存位置
    model_weights_save_path = '/home/giga/Desktop/davistao/p2m_tf2/Pixel2Mesh-Tensorflow2/results/coarse_mvp2m/models' \
    ...
    # 装饰器，用于将Python函数转化成Tensorflow计算图。使用该装饰器可以提高Tensorflow程序的性能
    # 训练函数
    ...
    def train_step(img_with_cameras, labels):
        with tf.GradientTape() as tape:
            ...
            loss = get_loss(output1, output2, output3, output1_2, output2_2, features, trainable_variables, labels,
                            edges, faces_triangle, lape_idx)
