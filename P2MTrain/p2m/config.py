...
import yaml
import re
# 从命令行参数和YAML配置文件中获取参数，并处理这些参数以便在程序中使用。
# 这种方法允许用户轻松地更改配置，而无需修改代码。
...
def execute(yaml_path):
    return parse_args(create_parser(yaml_path))


def create_parser(yaml_path):

    parser = argparse.ArgumentParser(description='参数初始化')
    # g = parser.add_argument_group('Device Targets')
    parser.add_argument('-f', '--config_file', dest='config_file', type=argparse.FileType(mode='r'),default=yaml_path)  #如果命令行有 -f，则将执行-f 内容
