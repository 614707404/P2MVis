...
class GraphConvolution(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.sparse_inputs:
            ...
            # TODO 此处原代码是 x = tf.nn.dropout(x, 1-self.dropout)
...
class GraphProjection(tf.keras.layers.Layer):
    ...
    def call(self, inputs,img_feat):
        ...
        return outputs
# 这个类是一个自定义的 Keras 层，用于将三维坐标数据投影到图像上。
...
# 这个类是一个自定义的 Keras 层，用于生成初始网格的局部网格采样。
# 给定一组三维网格坐标（mesh_coords），它将使用给定的偏移量（sample_coord）生成一组新的采样点。
# 这些采样点将用于下一阶段的形状推理。
...
# 这个类与 GraphProjection_plus 类非常相似，
# 它也是一个用于将三维坐标投影到图像上的自定义 Keras 层
