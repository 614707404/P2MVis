...
# 采样假设点
# 总之，SampleHypothesis 类是一个用于采样假设点的自定义 TensorFlow 层。它通过在输入的三维网格上添加采样偏移量（sample_coord）来生成新的采样点。这个过程有助于在计算机视觉和图形处理任务中提取有代表性的特征点。
...
class LocalGConv(tf.keras.layers.Layer):
    def __init__(self, input_dim, output_dim, sample_adj,dropout1=0,dropout=False, act=tf.nn.relu, bias=True, **kwargs):
    ...
    def call(self, inputs):
        ...
        supports = list()
        for i in range(len(self.support)):
            pre_sup = tf.einsum('ijk,kl->ijl', x, self.weights_var[i])
...
class DeformationReasoning(tf.keras.layers.Layer):
    def __init__(self, input_dim, sample_coord,sample_adj, args=None, **kwargs):
        super(DeformationReasoning, self).__init__(**kwargs)
        self.delta_coord = sample_coord
        self.s = 43
        self.f = args.stage2_feat_dim
        self.hidden_dim = 192
        self.local_conv1 = LocalGConv(input_dim=input_dim, output_dim=self.hidden_dim,sample_adj=sample_adj)
