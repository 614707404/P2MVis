...
class GCN_model(tf.keras.Model):
    ...
    def call(self, img_with_cameras):
        ...
        return self.output1, self.output2, self.output3, self.output1_2, self.output2_2, trainable_variables, self.img_feat
    ...
    def call_cnn(self, img_inp):
        ...
        # feat : feature
        self.img_feat = [tf.squeeze(x2), tf.squeeze(x3), tf.squeeze(x4), tf.squeeze(x5), tf.squeeze(x1), ]
        # 正则化损失函数无关
