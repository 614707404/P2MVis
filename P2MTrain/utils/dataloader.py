...
class DataFetcher(threading.Thread):
    ...
    dataroot:/home/giga/Desktop/davistao/p2m_tf2/Pixel2Mesh-Tensorflow2/data/ShapeNetModels/p2mppdata/test
    imageroot:/home/giga/Desktop/davistao/p2m_tf2/Pixel2Mesh-Tensorflow2/data/ShapeNetImages/ShapeNetRendering
    '''
    # file_list：包含要处理的文件名的列表
    # data_root：数据的根目录
    # image_root：图像的根目录
    # is_val：一个布尔值，表示是否在验证模式下运行
    # mesh_root：网格数据的根目录（可选）
    def __init__(self, file_list, data_root, image_root, is_val=False, mesh_root=None, special_data=None, special_interval=5):
    ...
    def run(self):
        ...
        while self.index < 9000000 and not self.stopped:
            if self.index % self.special_interval == 0 and self.special_data is not None:
                self.queue.put(self.special_data)
            else:
