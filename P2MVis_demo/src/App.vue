<template>
    <div class="show-container">
        <div class="navbar navbar-dark bg-dark top-panel">
            <a class="navbar-brand" href="#" style="display: flex; align-items: center;">
                <img src="logo_transparent_background.png" alt="Logo" style="height: 30px; margin-right: 10px;">
                <div style="font-weight: bold; font-size: 30px;">P2M Vis</div>
                <div style=" margin-left: 15px; margin-top: 10px; font-size: 20px;">Learn how Pixel2Mesh generates mesh
                    from
                    pixels</div>
            </a>
        </div>
...
<p>
    Interactive Feature Localization in Deep neural networks (IFeaLiD) is a web application that allows you to visualize
    and explore deep neural network layers or any hyperspectral image interactively in the browser. <a
        href="https://www.frontiersin.org/articles/10.3389/frai.2020.00049">Read the paper</a>.
</p>
<p>
    Code and information on how to generate a dataset ZIP file can be found at <a
        href="https://github.com/BiodataMiningGroup/IFeaLiD">GitHub</a>.
</p>
<p class="mb-0">
...
<template #modal-footer>
                ...
                <div class="middle-section">
                    <div id="visual-three">
                        <Visualization id="feature-visual" ref="visualization" v-bind:dataset="dataset"
                            ...
                            v-on:select="updateSelectPixelVector" :key="renderKey"></Visualization>
                        <div id="modle-visual">
                            <ThreeModelMouse :objPath="objPath" :key="renderKey"
                                @particleSelected="handleParticleSelected">
                            </ThreeModelMouse>
                        </div>
                    </div>
                </div>
        ...
        <!-- <input type="file" name="file" accept="application/zip" ref="fileInput" @change="selectedFile"
            style="display: none;"> -->
    </div>
</template>
...
import Parms from './components/Parms.vue';
...
export default {
    ...
    methods: {
        async loadData() {
            try {
                const response = await fetch('normal.json'); // 确保这里的路径与你的JSON文件位置相匹配
                if (!response.ok) {
                    throw new Error('Failed to load data');
                }
                this.parsedData = await response.json(); // 加载并解析JSON，然后更新data中的parsedData
            } catch (error) {
                console.error('Error loading JSON:', error);
            }
        },
        onIterationCountChanged(newCount) {
            // 在这里处理iterationCount变化
            // 例如，可以在这里发送请求、更新UI等
            ...
            if (dataset) {
                dataset.data.push(parseInt(this.parsedData[parseInt(this.iterationCount) - 1].Meanloss));
            }


            dataset = this.yData_2.find(d => d.name === "Chamfer Loss");
            if (dataset) {
                dataset.data.push(parseInt(this.parsedData[parseInt(this.iterationCount) - 1].p_loss));
            }

            dataset = this.yData_2.find(d => d.name === "Edge Regularization");
            if (dataset) {
                dataset.data.push(parseInt(this.parsedData[parseInt(this.iterationCount) - 1].e_loss));
            }

            dataset = this.yData_2.find(d => d.name === "Normal Loss");
            if (dataset) {
                dataset.data.push(parseInt(this.parsedData[parseInt(this.iterationCount) - 1].n_loss));
            }

            // dataset = this.yData_3.find(d => d.name === "m_loss");
            // if (dataset) {
            //     dataset.data.push(this.parsedData.m_loss);
            // }
        ...
        handleStartChange(newValue) {
            ...
            // this.iterationCount == "222";
            // 如果需要在某个条件下停止计时器，可以在这里调用 clearInterval(timer);
...
.show-container {
    ...
    .top-panel {
        // background-color: #ccc; /* 顶部颜色，用于可视化 */
        flex: 1;
        /* 顶部高度占比 */
    }
